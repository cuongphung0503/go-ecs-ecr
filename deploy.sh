...
run_task(){
#  	task_definition=$(echo $revision | jq '.taskDefinition.revision')
# 	echo "$task_definition"
    if run = $(aws ecs run-task --cluster circleci --task-definition "circleci:" $revision | jq '.taskDefinition.revision'); then
    	echo $run
    else 
  	echo "Failed"
	return 1
